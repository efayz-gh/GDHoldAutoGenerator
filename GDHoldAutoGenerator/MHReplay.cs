...
public class MHReplay : GDReplay
{
    ...
    public override GDObjectList ToObjects(bool clickSounds = false, string editorLayer = "", int linkId = 1)
    {
        var objects = new GDObjectList();
        ...
        }; // set initial options trigger to disable all controls

        if (editorLayer != string.Empty)
            o[Param.EditorLayer] = editorLayer;
        o[Param.EditorLayer2] = "0";
        ...
        foreach (var input in Inputs)
        {
            ...
            // mirror trigger on release for visual indicator in editor
            if (!input.Down)
                obj[Param.MirrorVertical] = "1";
            
            if (editorLayer != string.Empty)
                obj[Param.EditorLayer] = editorLayer;
            ...
            if (clickSounds)
            {
                objects.Add(new GDObject
                {
                    ...
                    [Param.LinkId] = linkId.ToString() // link both triggers to make manually moving easier
                });
                
                obj[Param.LinkId] = linkId++.ToString();
            }
